...
from doc_assistant.agent._planning import _parse_llm_plan, _trim_plan
from doc_assistant.agent.schemas import AgentPlanStep
...
def plan_task_with_llm(
    ...
    *,
    ...
    prompt = PLANNER_PROMPT.format(
        ...
        max_steps=max_steps,
        skill_instructions=(
            skill_context.rendered_instructions
            if skill_context and skill_context.rendered_instructions
            else "No runtime skills selected."
        ),
    )
    messages = [
    ...
    try:
        if isinstance(chat_model, BaseChatModel):
            ...
            raw = chat_model.invoke_messages(
                [
        ...
        elif isinstance(chat_model, InvokableChatModelProtocol):
            try:
