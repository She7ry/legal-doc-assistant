...
def verify_citation_support(answer: str, citations: list[Citation]) -> CitationSupportResult:
    ...
    for claim in _material_claims(answer):
        ...
        claim_text = strip_citations(claim).strip()
...
def _support_status(claim: str, source: str) -> tuple[str, str]:
    ...
    coverage = lexical_coverage(claim, source)
    overlap = claim_tokens & semantic_tokens(source)
...
def citation_ids(text: str) -> tuple[str, ...]:
    return tuple(dict.fromkeys(match.upper() for match in CITATION_PATTERN.findall(text)))


def strip_citations(text: str) -> str:
    return CITATION_PATTERN.sub("", text or "")


def lexical_coverage(claim: str, source: str) -> float:
    claim_tokens = semantic_tokens(strip_citations(claim))
    if not claim_tokens:
        return 1.0 if source.strip() else 0.0
    return len(claim_tokens & semantic_tokens(source)) / len(claim_tokens)
