...
class IngestJobStore:
    ...
    def create(self, user_id: str, file_name: str, saved_path: Path) -> IngestJobRecord:
    ...
    def get(self, job_id: str, user_id: str) -> IngestJobRecord | None:
        with self._lock:
        ...
        with self._connect() as connection:
            connection.execute(
                ...
                INSERT INTO ingest_jobs (
                    job_id, user_id, file_name, saved_path, status, stage, progress,
    ...
    def _update_record(self, record: IngestJobRecord) -> None:
        with self._connect() as connection:
            connection.execute(
                """
                UPDATE ingest_jobs
                SET user_id = ?, file_name = ?, saved_path = ?, status = ?, stage = ?,
