...
from ai.rag.ingestion.loader import SUPPORTED_EXTENSIONS
...
from backend.routers import agent, auth, chat, documents, review
from backend.schemas.responses import HealthCheckOut, HealthResponse
from backend.task_queue import shutdown_background_tasks
...
def health() -> HealthResponse:
    checks = [
        ...
        _path_check("auth_store", settings.auth_db_path.parent),
...
def _rate_limit_key(request: Request) -> str:
    ...
    if authorization.casefold().startswith("bearer "):
        return f"session:{sha256(authorization[7:].strip().encode()).hexdigest()}"
    session_token = request.cookies.get(SESSION_COOKIE_NAME)
    if session_token:
        return f"session:{sha256(session_token.encode()).hexdigest()}"
